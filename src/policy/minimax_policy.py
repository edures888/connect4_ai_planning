...
class MinimaxPolicy(BasePolicy):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if hasattr(batch.obs, "mask"):
            mask = batch.obs.mask
            # self.logger.debug("Action mask found:", mask)
        ...
        for i in range(len(obs)):
            ...
            if mask is not None:
                ...
                if obs_mask[best_action] == 0:
                    self.logger.debug(
                        f"WARNING: Action {best_action} is illegal according to the mask!"
                    )
                    self.logger.debug(f"Action mask: {obs_mask}")
                    ...
                    if len(legal_actions) > 0:
                        ...
                        self.logger.debug(
                            f"Choosing legal action {best_action} instead."
                        )
    ...
    def _obs_to_board(self, obs: Union[np.ndarray, dict, "Batch"]) -> np.ndarray:
        ...
        try:
            self.logger.debug(f"Observation shape: {getattr(obs, 'shape', 'unknown')}")

            if hasattr(obs, "shape") and obs.shape == (
                6,
                7,
                2,
            ):  # Assuming shape is (height, width, channels)
                player_pieces = obs[:, :, 0]
                ...
                self.logger.debug(f"Player pieces count: {np.sum(player_pieces == 1)}")
                self.logger.debug(
                    f"Opponent pieces count: {np.sum(opponent_pieces == 1)}"
                )

            elif hasattr(obs, "shape") and obs.shape == (
                2,
                6,
                7,
                ...
                self.logger.debug(f"Trying to infer format from shape: {obs.shape}")
                ...
                if obs.shape[0] == 2:  # Channels first
                    ...
                    self.logger.debug(f"Cannot infer format from shape: {obs.shape}")
                    return board
                ...
                self.logger.debug(
                    f"Warning: Unsupported observation shape: {getattr(obs, 'shape', 'unknown')}. Using empty board."
                )
                if hasattr(obs, "shape"):
                    self.logger.debug(
                        f"Observation content sample: {obs.flatten()[:10] if hasattr(obs, 'flatten') else 'cannot flatten'}"
                    )
        except Exception as e:
            # If there's an error processing the observation, log a warning and return an empty board
            self.logger.debug(
                f"Warning: Error processing observation: {e}. Using empty board."
            )
    ...
    def _get_best_action(self, board: np.ndarray) -> int:
        ...
        """
        self.logger.debug("=== MINIMAX DECISION MAKING ===")
        self.logger.debug(
            f"Player ID: {self.player_id}, Opponent ID: {self.opponent_id}"
        )
        self.logger.debug("Current board state:")
        ...
        if not valid_actions:
            self.logger.debug("No valid actions, returning random action")
        ...
        for action in valid_actions:
            ...
            if self._check_winner(new_board) == self.player_id:
                self.logger.debug(f"Found winning move at column {action}")
                return action
        ...
        for action in valid_actions:
            ...
            if winner == self.opponent_id:
                self.logger.debug(
                    f"Blocking opponent's winning move at column {action}"
                )
                self.logger.debug("Board if opponent plays here:")
                self._print_board(new_board)
                return action

        self.logger.debug("No immediate threats, proceeding with minimax")
        ...
        for action in valid_actions:
            ...
            value = self._minimax(
                new_board, self.max_depth - 1, float("-inf"), float("inf"), False
            )
            self.logger.debug(f"Action {action} has value {value}")
        ...
        self.logger.debug(f"Choosing action {best_action} with value {best_value}")
        return best_action

    def _print_board(self, board: np.ndarray) -> None:
        """self.logger.debug the board state for debugging.
        ...
        self.logger.debug("0 1 2 3 4 5 6")  # Column numbers
    ...
    def _minimax(
        ...
        if is_maximizing:
            ...
            for action in valid_actions:
                ...
                self._apply_action(new_board, action, self.opponent_id)
                value = min(
                    value, self._minimax(new_board, depth - 1, alpha, beta, True)
                )
                beta = min(beta, value)
                if alpha >= beta:
                    break  # Alpha cutoff
