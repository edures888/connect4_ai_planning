"""Adapted from original Tianshou Collector class"""
...
from src.policy.mcts import MCTS


class MCTSCollector(object):
    """Adds MCTS rollouts to guide action selection
    ...
    def __init__(
        ...
        self.mcts = MCTS(policy, self.env)
    ...
    def collect(
        ...
        while True:
            ...
            if random:
            ...
            else:  # MCTS for action selection
                if no_grad:
                    with torch.no_grad():  # faster than retain_grad version
                        # self.data.obs will be used by agent to get result
                        result = self._forward_mcts_action(self.data, last_state)
                ...
                # rely on MCTS for exploration instead of `exploration_noise`
            ...
            # print(f'\n========Step: {step_count}============')
            # print(f'Obs: {self.data.obs.obs.shape}') (num_env, board_row, board_col, players)
            # print(f'Obs MASK: {self.data.obs.mask.shape}')
            # print(f'Obs_next: {self.data.obs_next.obs.shape}')
